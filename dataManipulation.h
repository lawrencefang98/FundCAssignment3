...
void sortUserList(Clinic_t clinic);
int getPhoneNo();
...
/**
 * displayUserList
 * Displays all users in system sorted by first name
 * Sorted algorithm used is bubble sort
 * @param Clinic_t clinic: pointer to clinic
 **/
void displayUserList(Clinic_t *clinic)
{
    ...
    int i;
    int j;
    User_t sortedUserList[clinic->userListCount];
    for (i = 0; i < clinic->userListCount; i++)
    {
        sortedUserList[i] = clinic->userList[i];
    }
    User_t temp;
    for (i = 0; i < clinic->userListCount; i++) {
        for (j = 0; j < clinic->userListCount - 1 - i; j++) {
            if (strcmp(sortedUserList[j].firstName, sortedUserList[j + 1].firstName) > 0)
            {
                temp = sortedUserList[j];
                sortedUserList[j] = sortedUserList[j+1];
                sortedUserList[j+1] = temp;
            }
        }
    }

    printf("UserID | First Name | Last Name | Date of Birth | Email | Phone Number | Address | Gender\n");
    for (i = 0; i < clinic->userListCount; i++)
    {
        User_t userToDisplay = sortedUserList[i];
